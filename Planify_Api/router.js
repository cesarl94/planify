...
router.get("/board", async (httpReq, httpRes) => {
  ...
  for (let i = 0; i < estados.length; i++) {
    ...
    for (let j = 0; j < tareas_estadoSQL.length; j++) {
      ...
      const usuarios_tarea = usuarios_tareas.filter(
        (usuario_tarea) => usuario_tarea.id_tarea == tarea.id_tarea
      );
      const usuarios_filtered = usuarios.filter((usuario) =>
        usuarios_tarea.some(
          (usuario_tarea) => usuario_tarea.id_usuario == usuario.id_usuario
        )
      );
...
router.get("/task/:id", async (req, res) => {
  ...
  const task_users_ids = await new Promise((promRes, promRej) => {
    dbContainer.db.execute(
      `SELECT id_usuario FROM usuarios_tareas WHERE id_tarea = ?`,
      [id_tarea_integer],
      (sqlErr, sqlRes) => {
        if (sqlErr) {
          promRej(sqlErr.message);
...
router.get("/users", (req, res) => {
  const query =
    "SELECT id_usuario, nombre, apellido, foto_perfil FROM usuarios";
...
router.get("/user/:id", (req, res) => {
  ...
  dbContainer.db.execute(
    `SELECT id_usuario, nombre, apellido, foto_perfil FROM usuarios WHERE id_usuario = ?`,
    [user_id_integer],
    (err, results) => {
      if (err) {
        res.status(400).json({ error: err.message });
      ...
      if (results.length == 0) {
        res.status(400).json({
          error: `There isn't any user with the id: ${user_id_integer}`,
        });
        return;
      }
...
router.post("/newstate", (req, res) => {
  ...
  if (nombre == undefined || orden == undefined) {
    res
      .status(400)
      .json({ error: "Missing data. Required data: nombre, orden" });
    return;
  }
...
router.post("/newregister", async (req, res) => {
  ...
  if (
    ...
    res.status(400).json({
      error: "Missing data. Required data: correo, password, nombre, apellido",
    });
    return;
  }
  ...
  dbContainer.db.execute(
    `INSERT INTO usuarios (correo, hash, nombre, apellido) VALUES (?, ?, ?, ?)`,
...
router.post("/newtask", (req, res) => {
  ...
  if (nombre == undefined || orden == undefined || id_estado == undefined) {
    res
      .status(400)
      .json({ error: "Missing data. Required data: nombre, orden, id_estado" });
    return;
  }
  ...
  dbContainer.db.execute(
    `INSERT INTO tareas (nombre, orden, id_estado, fecha_creacion) VALUES (?, ?, ?, ?)`,
    [nombre, orden, id_estado, mysqlDate],
    (err, results) => {
      if (err) {
        res.status(400).json({ error: err.message });
        return;
...
router.patch("/updatestate", (req, res) => {
  ...
  if (id_estado == undefined || (orden == undefined && nombre == undefined)) {
    res.status(400).json({
      error: "Missing data. Required data: id_estado && (orden || nombre)",
    });
    return;
  }
  ...
  if (id_estado < 1) {
    res
      .status(400)
      .json({ error: "The value 'id_estado' can't be less than 1" });
    return;
  }
  ...
  dbContainer.db.execute(
    `UPDATE estados SET ${ordenQuery}${optionalComma}${nombreQuery} WHERE id_estado = ?`,
    params,
    (err, results) => {
      if (err) {
        res.status(400).json({ error: err.message });
      ...
      if (results.affectedRows == 0) {
        res.status(400).json({
          error: "Provided value 'id_estado' isn't in our data base.",
        });
        return;
      }
...
router.patch("/updatetask", async (req, res) => {
  ...
  if (
    ...
    res.status(400).json({
      error:
        "Missing data. Required data: id_tarea && (nombre || descripcion || fecha_limite || orden || prioridad || id_estado || usuarios)",
    });
    return;
  }
  ...
  if (prioridad != undefined) {
    if (prioridad < 0 || prioridad > 5) {
      res
        .status(400)
        .json({ error: "Value 'prioridad' must be between 0 and 5" });
      return;
    }
  ...
  if (keys.length > 0) {
    ...
    const sql_response = await new Promise((promRes, promRej) => {
      dbContainer.db.execute(
        `UPDATE tareas SET ${keysString} WHERE id_tarea = ?`,
        params,
        (sqlErr, sqlRes) => {
          if (sqlErr) {
            promRej(sqlErr.message);
  ...
  const usersToDelete = task_users_ids.filter(
    (task_user_id) => !usuarios.some((usuario) => usuario == task_user_id)
  );
  const usersToCreate = usuarios.filter(
    (usuario) => !task_users_ids.some((task_user_id) => task_user_id == usuario)
  );
  ...
  if (usersToCreate.length > 0) {
    const insertQuery = `
        ...
        `;
    const insertParams = usersToCreate.flatMap((usuario) => [
      usuario,
      id_tarea,
    ]);
  ...
  if (usersToDelete.length > 0) {
    const deleteQuery = `
            ...
            WHERE id_tarea = ? AND id_usuario IN (${usersToDelete
              .map(() => "?")
              .join(" , ")})
        `;
...
router.delete("/task/:id", async (req, res) => {
  ...
  dbContainer.db.execute(
    `DELETE FROM usuarios_tareas WHERE id_tarea = ?`,
    [id_tarea_integer],
    (sqlErr, sqlRes) => {
      if (sqlErr) {
...
router.delete("/state/:id", async (req, res) => {
  ...
  const tasksInState = await new Promise((promRes, promRej) => {
    dbContainer.db.execute(
      `SELECT id_tarea FROM tareas WHERE id_estado = ?`,
      [id_estado_integer],
      (sqlErr, sqlRes) => {
        if (sqlErr) {
          const errorMessage = sqlErr.message;
  ...
  if (tasksInState.length > 0) {
    ...
    const deleteUsersTasksResponse = await new Promise((promRes, promRej) => {
      dbContainer.db.execute(
        `DELETE FROM usuarios_tareas WHERE id_tarea IN ${values}`,
        tasksInState,
        (sqlErr, sqlRes) => {
          if (sqlErr) {
            const errorMessage = sqlErr.message;
      ...
      console.log(
        "Error on DELETE /state/:id from DELETE USUARIOS_TAREAS:",
        errorMessage
      );
      return null;
    });
    ...
    const deleteTaskResponse = await new Promise((promRes, promRej) => {
      dbContainer.db.execute(
        `DELETE FROM tareas WHERE id_tarea IN ${values}`,
        tasksInState,
        (sqlErr, sqlRes) => {
          if (sqlErr) {
            const errorMessage = sqlErr.message;
      ...
      console.log(
        "Error on DELETE /state/:id from DELETE TAREAS:",
        errorMessage
      );
      return null;
    });
  ...
  const deleteStateResponse = await new Promise((promRes, promRej) => {
    ...
    console.log(
      "Error on DELETE /state/:id from DELETE ESTADOS:",
      errorMessage
    );
    return null;
  });
...
router.patch("/tareas/:id/estado", (req, res) => {
  ...
  const query = `UPDATE tareas SET id_estado = ? WHERE id_tarea = ?`;

  dbContainer.db.execute(query, [id_estado, id_tarea], (err, results) => {
    if (err) {
      res.status(400).json({ error: err.message });
      return;
    }
